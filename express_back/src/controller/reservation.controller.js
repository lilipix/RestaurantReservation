...
class ReservationController {
  ...
  static async getReservationsByRestaurant(req, res, next) {
    try {
      ...
      const reservations =
        await ReservationService.getReservationsByRestaurant(restaurantId);
      ApiResponse.success(
        res,
        "Reservations retrieved for restaurant",
        reservations,
      );
    } catch (error) {
      next(error);
  ...
  static async createReservation(req, res, next) {
    try {
      ...
      if (!body.restaurant)
        return ApiResponse.badRequest(res, "restaurant is required");
      if (!body.customerName)
        return ApiResponse.badRequest(res, "customerName is required");
      if (!body.date)
        return ApiResponse.badRequest(res, "date is required (YYYY-MM-DD)");
      if (!body.time)
        return ApiResponse.badRequest(res, "time is required (HH:mm)");
  ...
  static async updateReservation(req, res, next) {
    try {
      ...
      const existing = await ReservationService.getReservationById(
        restaurantId,
        reservationId,
      );
      ...
      if (String(existing.restaurant._id) !== String(restaurantId)) {
        return ApiResponse.badRequest(
          res,
          "Reservation does not belong to the specified restaurant",
        );
      }
  ...
  static async deleteReservation(req, res, next) {
    try {
      ...
      await ReservationService.deleteReservation(restaurantId, reservationId);
